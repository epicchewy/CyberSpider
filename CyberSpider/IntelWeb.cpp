...
bool IntelWeb::createNew(const std::string &filePrefix, unsigned int maxDataItems){
    ...
    unsigned int hashMapSize = maxDataItems / .75; //applies load factor to the hashtables
    bool check1 = m_in.createNew(temp1, hashMapSize);
    bool check2 = m_out.createNew(temp2, hashMapSize);
    
    if(check1 && check2){ //if both are successfully written return true
        m_ready = true;
        return true;
    }else{ //if not close the intelweb file and return false
        close();
        return false;
    }
}
...
bool IntelWeb::openExisting(const std::string &filePrefix){
    ...
    m_in.close(); //close current diskmultimaps
    m_out.close();
    
    string temp1 = filePrefix + "_in.dat";
    string temp2 = filePrefix + "_out.dat";
    
    bool check1 = m_in.openExisting(temp1); //open the hashtables
    bool check2 = m_out.openExisting(temp2);
    
    if(check1 && check2){ //if successfully opened return true;
        m_ready = true;
        return true;
    }else{
        close();
...
bool IntelWeb::ingest(const std::string& telemetryFile){
    ...
    while (getline(inf, line))
    {
        ...
        m_in.insert(key, value, context);
        m_out.insert(value, key, context);
    }
    return true;
}
...
unsigned int IntelWeb::crawl(const std::vector<std::string>& indicators, unsigned int minPrevalenceToBeGood, std::vector<std::string>& badEntitiesFound, std::vector<InteractionTuple>& badInteractions){
    ...
    queue<string> qIndicators; //queue is needed for the bfs
    set<string> maliciousItems;
    set<InteractionTuple, compare_interactionTuple> interactions; //set takes in to objects so that it will automatically compare values and sort
    unordered_map<string, bool> m_marks; //tracker
    
    for(auto it = indicators.begin(); it != indicators.end(); it++){ //push in all the malicious entities into the queue
        m_marks[*it] = true;
    ...
    while(!qIndicators.empty()){ //find all the malicious items
...
bool IntelWeb::purge(const std::string& entity){
    ...
    list<InteractionTuple> input; //get all the references to possibly purge
    list<InteractionTuple> output;
    
    auto iterator_in = m_in.search(entity); //we need both iterators for this function -- one to go through the input file and one to go through the output file
    ...
    //remove the entities from the disk files themselves
    ...
    return purged; //return true if entities were successfully purged
}


unsigned int IntelWeb::getBadCount(const std::string& key, int min){ //this function gets the prevalence of the bad  item
    auto it = m_badObjects.find(key);
    
    if(it == m_badObjects.end()){ //find all the items in the cache and the m_badObjects vector that are bad
        int numBad = 0;
        ...
        while(output_it.isValid()){ //do this for both the input and output files
